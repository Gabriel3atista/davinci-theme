 ...
 * @prop {Record<'base'|'ansi'|'brightOther'|'other', string[]>} davinci - davinci color variables.
...
const transformSoft = theme => {
    ...
    const brightColors = [...soft.davinci.ansi, ...soft.davinci.brightOther];
...
module.exports = async () => {
    const yamlFile = await readFile(
        join(__dirname, '..', 'src', 'davinci.yml'),
        'utf-8'
    );
